var testCase = require("nodeunit").testCase,
  ...
  static_document = require("fs").readFileSync(
...
module.exports = testCase({
  ...
  "jQuery()": function (test) {
    ...
    test.equals(
      jQuery(undefined).length,
      0,
      "jQuery(undefined) === jQuery([])"
    );
    ...
    test.equals(
      code.length,
      1,
      "Correct number of elements generated for code"
    );
    test.equals(
      code.parent().length,
      0,
  ...
  "selector state": function (test) {
    ...
    test.equals(
      elem.selector,
      "#main.filter(div).not(div)",
      "#main filter, not Selector"
    );
  ...
  "text()": function (test) {
    ...
    test.equals(
      ...
      jQuery("#sndp").text(),
      "check for text in complex paragraph"
    );
    test.done();
  },
  "end()": function (test) {
  ...
  "attr()": function (test) {
    ...
    test.expect(4);
    test.equals(
      jQuery("#input1").attr("name"),
      "PWD",
      "Get form element name attribute"
    );
    test.equals(
      jQuery("#input2").attr("name"),
      "T1",
      "Get form element name attribute"
    );
    test.equals(
      jQuery("item").attr("name"),
      "test val",
      "Get name attribute from element"
    );
    e = jsdom('<element name="dude" age="25">content</element>');
    test.equals(
      jQuery("element", e).attr("name"),
      "dude",
      "Get name attribute from element"
    );
    test.done();
  ...
  "map()": function (test) {
    ...
    test.equals(
      values.join(""),
      "12",
      "Map the values from a hash to an array"
    );

    var scripts = document.getElementsByTagName("script");
    var mapped = jQuery.map(
      scripts,
      function (v, k) {
        return v;
      },
    ...
    test.equals(
      flat.join(""),
      "00012223",
      "try the new flatten technique(#2616)"
    );
  },
  ...
  "jQuery.extend(Object, Object)": function (test) {
    ...
    // #5991
    test.ok(
      jQuery.isArray(jQuery.extend(true, { arr: {} }, nestedarray).arr),
      "Cloned array heve to be an Array"
    );
    test.ok(
      jQuery.isPlainObject(jQuery.extend(true, { arr: arr }, { arr: {} }).arr),
      "Cloned object heve to be an plain object"
    );
    ...
    test.equals(
      obj.foo,
      "notnull",
      "Make sure a null value can be overwritten"
    );
  ...
  "jQuery.makeArray": function (test) {
    ...
    test.equals(
      ...
      0,
      "Pass nothing to makeArray and test.expect an empty array"
    );
  ...
  "jQuery.parseJSON": function (test) {
    ...
    test.same(
      ...
      { test: 1 },
      "Plain object parsing."
    );

    test.same(
      jQuery.parseJSON('\n{"test":1}'),
      { test: 1 },
      "Make sure leading whitespaces are handled."
    );
  ...
  "jQuery.sub() - Static Methods": function (test) {
    ...
    test.ok(
      Subclass.topLevelMethod() === false,
      "Subclass.topLevelMethod thought debug was true"
    );
    test.ok(
      Subclass.config.locale == "en_US",
      Subclass.config.locale + " is wrong!"
    );
    test.same(
      Subclass.config.test,
      undefined,
    ...
    test.equal(
      jQuery.ajax,
      Subclass.ajax,
      "The subclass failed to get all top level methods"
    );
    ...
    test.ok(
      SubSubclass.topLevelMethod() === false,
      "SubSubclass.topLevelMethod thought debug was true"
    );
    test.ok(
      SubSubclass.config.locale == "en_US",
      SubSubclass.config.locale + " is wrong!"
    );
    test.same(
      SubSubclass.config.test,
      undefined,
    ...
    test.ok(
      ...
      "SubSubclass.topLevelMethod thought debug was false"
    );
    test.same(
      SubSubclass(document).subClassMethod,
      Subclass.fn.subClassMethod,
      "Methods Differ!"
    );
    test.ok(
      SubSubclass.config.locale == "es_MX",
      SubSubclass.config.locale + " is wrong!"
    );
    test.ok(
      SubSubclass.config.test == "worked",
      "SubSubclass.config.test is set incorrectly"
    );
    test.notEqual(
      jQuery.ajax,
    ...
    test.same(
      Subclass(document).subSubClassMethod,
      undefined,
      "subSubClassMethod set incorrectly"
    );
    test.equal(
      jQuery.ajax,
      Subclass.ajax,
  ...
  "jQuery.sub() - .fn Methods": function (test) {
    ...
    methods = [
      // all methods that return a new jQuery instance
      ["eq", 1],
      ["add", document],
      ["end"],
      ["has"],
    ...
    jQuery.each(selectors, function (i, selector) {
      jQuery.each(methods, function () {
        ...
        jQuery.each(contexts, function (i, context) {
          ...
          test.same(
            jQuery(selector, context)[method](arg).subclassMethod,
            undefined,
            "jQuery" + description + " doesnt have Subclass methods"
          );
          test.same(
            jQuery(selector, context)[method](arg).subclassSubclassMethod,
            undefined,
            "jQuery" + description + " doesnt have SubclassSubclass methods"
          );
          test.same(
            Subclass(selector, context)[method](arg).subclassMethod,
            Subclass.fn.subclassMethod,
            "Subclass" + description + " has Subclass methods"
          );
          test.same(
            Subclass(selector, context)[method](arg).subclassSubclassMethod,
            undefined,
            "Subclass" + description + " doesnt have SubclassSubclass methods"
          );
          test.same(
            SubclassSubclass(selector, context)[method](arg).subclassMethod,
            Subclass.fn.subclassMethod,
            "SubclassSubclass" + description + " has Subclass methods"
          );
          test.same(
            SubclassSubclass(selector, context)[method](arg)
              .subclassSubclassMethod,
            SubclassSubclass.fn.subclassSubclassMethod,
