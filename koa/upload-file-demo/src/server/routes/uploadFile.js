  ...
  fragmentation(ctx) {
    try {
      const body = ctx.request.body,
        { type, chunkNum, chunkCount, fileName } = body;
      ...
      if (type && type === "chunk") {
        ...
        const uploadDir = path.resolve(__dirname, "../../static/uploads"),
          ...
          filterFileList = fileList.filter((file) =>
            file.includes(_parse.name)
          );
        ...
        ctx.body = {
          ...
          fileUrl: config.uploadHost + newFileName,
        };
      } else {
    ...
    files.forEach((item) => {
      ...
      if (item.size > 0 && oldPath) {
        const oldFilePath = path.dirname(oldPath), // 存放文件的原始路径（/root/to/path）
          ...
          newPath = path.join(
            oldFilePath,
            srcFileName + "_" + oldFileName + ext
          ); // 生成新的文件路径
        // 重命名文件 （koa-body已经自动写入了文件）
        fs.renameSync(oldPath, newPath);

        result.push(
          config.uploadHost + path.join(srcFileName + "_" + oldFileName + ext)
        );
      }
    });
