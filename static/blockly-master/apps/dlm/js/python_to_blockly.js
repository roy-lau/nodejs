            ...
            for (var i = 0; i < args.length; i++) {
                ...
                if (typeof argument ==  "string") {
                    ...
                    if (argument.type == 'mutation') {
                        ...
                        desxiaodition[argument.name] = this.Num_value(args[i]);
                    } else if (argument.type == 'variable') {
                        if(argument.is_math_angle === true){
                            desxiaodition[argument.name] = this.convert(args[i].left.left);
                        }else{
                            desxiaodition[argument.name] = this.convert(args[i]);
                        }
                    } else if (argument.type == "integer_mapper") {
                        // Okay we jumped the shark here
                        var argumentName = argument.name;
                        var argumentMapper = argument.method;
                        desxiaodition[argumentName] = argumentMapper(this.Num_value(args[i]));
                    } else if (argument.type == 'mapper') {
                        var argumentName = argument.name;
                        var argumentMapper = argument.method;
                        desxiaodition[argumentName] = argumentMapper(this.Str_value(args[i]));
                    }
                } else {
