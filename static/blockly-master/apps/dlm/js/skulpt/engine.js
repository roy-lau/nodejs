 ...
 */
var PyEngine = function(programStatus, dlmProject) {
    this.loadEngine();
	this.programStatus = programStatus;
	this.dlmProject = dlmProject;
}
...
PyEngine.prototype.readFile = function(filename) {
	...
	if(this.dlmProject.exist(newFilename)){
		return this.dlmProject.getFileContent(newFilename);
	}
...
PyEngine.prototype.fileread = function(filename, mode) {
	if(this.dlmProject.exist(filename)){
    	return this.dlmProject.getFileContent(filename);
	}else{
		if(mode.indexOf('w') !== -1){
			this.dlmProject.add(filename, '', 1);
			return '';
		}else
			return null;	
	}
...
PyEngine.prototype.filewrite = function(fileItem, str) {
	...
	this.dlmProject.modify(filename, str);
	this.dlmProject.select(filename);
}
