...
const workbook = XLSX.readFile('胰腺癌单病种数据元2016（含随访）——完成.xlsx',{ cellDates: true,dateNF:'YYYY-MM-dd'});
...
function generate_PAT_SD_ITEM_RESULT() {
    ...
    SQL += `INSERT INTO PAT_SD_ITEM_RESULT(PATIENT_NO,SD_CODE,SD_ITEM_CODE,SD_ITEM_VALUE) VALUES `
...
function generate_PAT_DRAINAGE_TUBE() {
    ...
    SQL += `INSERT INTO PAT_DRAINAGE_TUBE(SD_CODE,PATIENT_NO,TUBE_NAME,RETENTION_DAYS,POD1,POD3,POD7,AMY_POD1,AMY_POD3,AMY_POD7,AMY_POD_DRAW,CREATE_DATE_TIME) VALUES `

    sheetJson.forEach(item => {
        if (item.TUBE_NAME) {
            SQL += `('${item.SD_CODE}','${item.PATIENT_NO}','${item.TUBE_NAME}',${item.RETENTION_DAYS||null},'${item.POD1}','${item.POD3}','${item.POD7}','${item.AMY_POD1}','${item.AMY_POD3}','${item.AMY_POD7}','${item.AMY_POD_DRAW}','${item.CREATE_DATE_TIME}') \n`
        }
    })
    SQL+=';\n'
}
...
function generate_PAT_FOLLOW_UP() {
    ...
    SQL += `INSERT INTO PAT_FOLLOW_UP(SD_CODE,PATIENT_NO,FU_TIMES,FOLLOW_UP_DATE,FOLLOW_UP_MONTHS,FU_REASON) VALUES `
...
function generate_PAT_FOLLOW_UP_RESULT() {
    ...
    SQL += `INSERT INTO PAT_FOLLOW_UP_RESULT(SD_CODE,PATIENT_NO,FU_TIMES,SD_ITEM_CODE,SD_ITEM_U_VALUE) VALUES `
...
function generate_PAT_FOLLOW_UP_TREAT() {
    ...
    SQL += `INSERT INTO PAT_FOLLOW_UP_TREAT(SD_CODE,PATIENT_NO,FU_TIMES,TREAT_NAME,DRUG_DOSE,TREAT_EFFECT,TREAT_COST,CA199_FRONT,CEA_FRONT,CA125_FRONT,TREAT_EVALUTE_FRONT,CA199_AFTER,CEA_AFTER,CA125_AFTER,TREAT_EVALUTE_AFTER,CREATE_DATE_TIME,TREAT_CYCLE,DRUG_NAME_TRADE) VALUES `
