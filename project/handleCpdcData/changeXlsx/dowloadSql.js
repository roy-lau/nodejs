...
const dowPATIENT_NO = async () => {
    try {
        ...
        await sql.connect('mssql://sa:sa@123@192.168.1.253/RYCPDC_C20190827')

        let patientMaps = []
        for (let i = 0; i < PAT_VISIT.length; i++) {
            console.log(i,PAT_VISIT.length)
            const list = await sql.query `SELECT * FROM [dbo].[PAT_VISIT] WHERE PATIENT_ID=${PAT_VISIT[i]['病案号']} AND SD_CODE='YXA_O'`
