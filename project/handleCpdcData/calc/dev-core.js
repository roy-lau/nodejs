    ...
    async _needFollowCount(key) {
        ...
        try {
            ...
            if (isDead == '1') { // 院内死亡
                ...
                // 查询患者随访死亡日期（录入员问：我们有些患者不知死亡，答：大致时间应知）
                const listFollowbyDeadOfDate = await pool.query `SELECT * FROM [dbo].[PAT_FOLLOW_UP_RESULT] WHERE PATIENT_NO=${key} AND SD_ITEM_CODE='YXA_O_257' AND SD_ITEM_VALUE!=''`,
                    retFollowbyDeadOfDate = listFollowbyDeadOfDate.recordset
                    console.log(retFollowbyDeadOfDate)
                    if (retFollowbyDeadOfDate.length) {
...
async function run() {
    const calc = new CALC(['345bda4fda79e57b', '12e9717c4072c91a', '179aa12a972196d9', '6c45d5448d05847a'])
