...
统计：
	...
	b 人数：7
	c 人数: 0 	全库共 14
	d，e 所有术后辅助治疗中 HRR阳性或者BRCA+PALB2阳性患者人数: 0

	所有术后辅助治疗中拿到OS占比： 71.5%
	a 中拿到OS占比：76.0%
	b 中拿到OS占比：72.1%
	c 中拿到OS占比：0%
	d，e 中拿到OS占比：0%

2. 使用含铂方案 （FOLFIRINOX，顺铂，奥沙利铂等）的晚期胰腺癌患者人数及占所有术后辅助化疗患者的比例（a）:
...
```sql
-- 免疫组化中的 “其他” 字段
SELECT * FROM [dbo].[PAT_SD_ITEM_RESULT] WHERE SD_ITEM_CODE='YXA_O_242' AND SD_ITEM_VALUE like '%HRR%'
SELECT * FROM [dbo].[PAT_SD_ITEM_RESULT] WHERE SD_ITEM_CODE='YXA_O_242' AND SD_ITEM_VALUE like '%BRCA%'
SELECT * FROM [dbo].[PAT_SD_ITEM_RESULT] WHERE SD_ITEM_CODE='YXA_O_242' AND SD_ITEM_VALUE like '%PALB%'
```

```sql
SELECT * FROM [dbo].[PAT_FOLLOW_UP_TREAT] WHERE concat(DRUG_NAME, DRUG_NAME_TRADE,DRUG_DOSE) like '%帕利%'

SELECT * FROM [dbo].[PAT_FOLLOW_UP_TREAT] WHERE DRUG_NAME like '%帕利%' AND DRUG_NAME_TRADE like '%帕利%' AND DRUG_DOSE like '%帕利%'

SELECT * FROM [dbo].[PAT_FOLLOW_UP_TREAT] WHERE DRUG_NAME like '%Ola%' AND DRUG_NAME_TRADE like '%Ola%' AND DRUG_DOSE like '%Ola%'

SELECT * FROM [dbo].[PAT_FOLLOW_UP_TREAT] WHERE DRUG_NAME like '%普卓%' AND DRUG_NAME_TRADE like '%普卓%' AND DRUG_DOSE like '%普卓%'


SELECT * FROM [dbo].[PAT_SD_ITEM_RESULT] WHERE SD_ITEM_CODE IN ('YXA_O_118','YXA_O_135','YXA_O_119','YXA_O_123','YXA_O_905','YXA_O_906','YXA_O_136''YXA_O_301','YXA_O_302','YXA_O_303','YXA_O_309','YXA_O_310','YXA_O_311','YXA_O_315') AND SD_ITEM_VALUE like '%普卓%'

SELECT * FROM [dbo].[PAT_SD_ITEM_RESULT] WHERE SD_ITEM_CODE IN ('YXA_O_118','YXA_O_135','YXA_O_119','YXA_O_123','YXA_O_905','YXA_O_906','YXA_O_136''YXA_O_301','YXA_O_302','YXA_O_303','YXA_O_309','YXA_O_310','YXA_O_311','YXA_O_315')  AND SD_ITEM_VALUE like '%Ola%'

SELECT * FROM [dbo].[PAT_SD_ITEM_RESULT] WHERE SD_ITEM_CODE IN ('YXA_O_118','YXA_O_135','YXA_O_119','YXA_O_123','YXA_O_905','YXA_O_906','YXA_O_136''YXA_O_301','YXA_O_302','YXA_O_303','YXA_O_309','YXA_O_310','YXA_O_311','YXA_O_315')  AND SD_ITEM_VALUE like '%帕利%'
-- 合并
SELECT * FROM [dbo].[PAT_SD_ITEM_RESULT] WHERE SD_ITEM_CODE IN ('YXA_O_118','YXA_O_135','YXA_O_119','YXA_O_123','YXA_O_905','YXA_O_906','YXA_O_136''YXA_O_301','YXA_O_302','YXA_O_303','YXA_O_309','YXA_O_310','YXA_O_311','YXA_O_315')  AND SD_ITEM_VALUE like concat('%铂%', '%普卓%','%Ola%') 
```
