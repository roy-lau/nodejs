...
const dowPATIENT_NO = async () => {
    try {
        ...
        for (let i = 0; i < PAT_VISIT.length; i++) {
            ...
            const list = await mssql.query(`SELECT * FROM [dbo].[PAT_VISIT] WHERE PATIENT_ID=${PAT_VISIT[i]['病案号']} AND SD_CODE='YXA_O'`)
...
const dowSql = async () => {
    try {
        const data = await mssql.query(`SELECT ITEM_CODE,ITEM_NAME,ITEM_CV_CODE,ITEM_UNIT FROM [dbo].[SD_ITEM_DICT] WHERE SD_CODE!='YXA'`)
        const list = await mssql.query(`SELECT * FROM [dbo].[SD_ITEM_CV_DICT] WHERE SD_CODE!='YXA'`)
